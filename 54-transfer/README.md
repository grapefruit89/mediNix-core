...
- **Mover Safety Safeguards:** Active-handle check via `lsof -t` avoids touching files actively written by unpackers; a 5-minute age buffer (`-mmin +5`) ensures write finality; atomic moves via `.staging_mover` prevent partially transferred files; source files are deleted only upon verified transfer success.
