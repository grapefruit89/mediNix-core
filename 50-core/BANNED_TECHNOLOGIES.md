...
| **Unrestricted `sudo`** | Whitelisted `sudo` rules | We DO use `sudo`, but only strictly for whitelisted commands (e.g., `media-admin` restart scripts). Passwordless full-root `sudo ALL=(ALL) NOPASSWD: ALL` is strictly banned. |
