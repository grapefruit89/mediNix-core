...
1. `scripts/scan_inconsistencies.py` — walks the repo, checks every `NNN-*.nix`
   ...
   ports) may remain.
2. `scripts/scan_duplicates.py` — full duplicate probe: same filename in different
...
## Support files
- `scripts/scan_inconsistencies.py` — deterministic consistency probe (run it).
- `scripts/scan_duplicates.py` — duplicate/collision probe (run it too).
